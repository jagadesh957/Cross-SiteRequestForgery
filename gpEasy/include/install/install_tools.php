...
class Install_Tools{
	...
	function Form_UserDetails(){
		...
		echo '<tr><td><b>'.$langmessage['Admin_Username'].'*</b></td><td><input type="text" class="text" name="username" value="'.htmlspecialchars($_POST['username']).'" /></td></tr>';
		echo '<tr><td><b>'.$langmessage['Admin_Password'].'*</b></td><td><input type="password" class="text" name="password" value="" /></td></tr>';
		echo '<tr><td><b>'.$langmessage['Repeat_Password'].'*</b></td><td><input type="password" class="text" name="password1" value="" /></td></tr>';
	}
	...
	function Form_Configuration(){
		...
		//Hide gpEasy Link
		echo '<tr><th colspan="2"><a href="javascript:toggleOptions()">'.$langmessage['options'].'...</a></th></tr>';
	...
	function Install_DataFiles( $destination = false, $language = 'en' ){
		...
		if( isset($args['hidegplink']) && $args['hidegplink'] == 'hide' ){
		...
		if( function_exists('sha1') ){
			...
			$users[$args['username']]['password'] = md5(trim($args['password']));
		}
